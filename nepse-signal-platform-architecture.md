...
**Stack:** Java 25 (LTS) · Spring Boot 4.0 (Spring Framework 7, Jakarta EE 11) · Spring Batch · PostgreSQL/TimescaleDB · Redis · Angular 22
...
### F7. Charting
- **Purpose:** One composite payload that drives the price chart with overlays and signal markers.
- **Inputs:** symbol, interval, date range, requested indicators/overlays.
- **Processing:** assemble candles + requested indicator series + volume-profile bins + signal markers into a single response.
- **Outputs:** chart-ready JSON (consumed by TradingView Lightweight Charts / ECharts in Angular).
- **Spec notes:** read-only composition over F3–F5; cache-friendly; volume profile delivered as horizontal histogram with POC/VAH/VAL annotations.
...
**Ta4j vs custom:** S1, S2, S3, S5, S6, S7 are built from Ta4j `Indicator`s + `Rule`s. **S4 (volume profile)** and **S8 (broker flow)** are custom — they read floorsheet-derived structures Ta4j has no concept of — and are exposed to the confluence layer as ordinary votes so they blend on equal footing. S9 (confluence) is this platform's own scorer.
...
- Optional regime gate: in a strong down-market (breadth/ADX), raise `T_buy` to avoid catching falling knives.
...
```sql
-- reference
instrument(symbol PK, name, sector, listed_shares, status, price_band, created_at)
...
alert_rule(id PK, user_id FK, type, params JSONB, enabled)
...
| Method | Path | Purpose |
|---|---|---|
| GET | `/api/v1/indicators/catalog` | Available indicators + params |
| GET | `/api/v1/indicators` | Series for `symbol`/`name`/`params`/range |
| GET | `/api/v1/indicators/{symbol}/latest` | Latest indicator snapshot |
| POST | `/api/v1/indicators/compute` | Ad-hoc compute (custom params) for overlays |
...
| Method | Path | Purpose |
|---|---|---|
| GET | `/api/v1/charts/{symbol}` | Composite payload: candles + `indicators=` overlays + `overlays=volprofile` + signal markers |
...
- Add `spring-boot-starter-batch` (pipeline orchestration), `org.ta4j:ta4j-core:0.22.1` (indicators + rules + backtesting), and `archunit-junit5` (test scope, boundary rules); Actuator + Micrometer for observability.
...
### 10.8 Step 8 — Charting API
- Composite endpoint assembling candles + overlays + volume profile + signal markers; ETag/caching.

### 10.9 Step 9 — Notifications & realtime
...
### 10.12 Step 12 — Observability, caching, testing
...
- **Charting:** TradingView Lightweight Charts (or ECharts) fed by the `/charts/{symbol}` composite payload; volume profile drawn as a right-aligned horizontal histogram with POC/VAH/VAL guides; signal markers on candles, click → reasons panel.
...
- **Ta4j-core 0.22.1** — released 2026-04-01; pure Java (deps: slf4j, commons-math3, gson), runs on Java 8+ so Java 25 is fine; 130+ indicators, composable `Rule`/`Strategy`, `BarSeriesManager`/`BacktestExecutor` with `AnalysisCriterion`s, `DecimalNum`/`DoubleNum` precision, thread-safe `CachedIndicator`s and native multithreaded backtesting.
